...
async def main():
    ...
    print("[MAIN] ========================================")
    print("[MAIN] Попытка остановить все предыдущие обновления...")
    try:
        await bot.delete_webhook(drop_pending_updates=True)
        print("[MAIN] ✅ Webhook удален, обновления очищены")
    except Exception as e:
        print(f"[MAIN] ⚠️ Не удалось удалить webhook: {e}")
    
    # Пытаемся остановить polling через API
    try:
        print("[MAIN] Пытаюсь остановить все активные getUpdates...")
        # Отправляем пустой запрос для остановки
    ...
    while retry_count < max_retries:
        try:
            ...
            print(f"[MAIN] ⚠️ ОСТАНОВИТЕ все другие экземпляры бота!")
            print(f"[MAIN] ⚠️ Проверьте:")
            print(f"[MAIN]   - Другие запущенные процессы Python")
            print(f"[MAIN]   - Другие контейнеры/сервисы")
            print(f"[MAIN]   - Другие скрипты с этим ботом")
            ...
            if retry_count < max_retries:
                ...
                print(f"[MAIN] ⚠️ Polling НЕ запущен из-за конфликта!")
                print(f"[MAIN] ✅ Планировщик продолжает работать!")
                print(f"[MAIN] 📋 Планировщик будет публиковать видео по расписанию")
                print(f"[MAIN] ❌ Бот НЕ будет отвечать на сообщения до решения конфликта")
                print(f"[MAIN] ========================================")
                ...
                while True:
                    ...
                    if iteration % 5 == 0:  # Каждые 5 минут
                        print(f"[MAIN] ⏰ Планировщик работает... (polling недоступен, прошло {iteration} мин)")
                        print(f"[MAIN] ⚠️ Остановите другие экземпляры бота для восстановления polling")
        except Exception as e:
